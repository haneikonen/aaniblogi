		...
		<input type="text" name="search" placeholder="Search... " />
     	<input type="submit" name="go" value="Go" class="button" />
			...
			for ($i = 1; $i <= $lkm_data['COUNT(ID)']; $i++) {
				$popularity = $DBH->prepare("SELECT COUNT(julkaisu), julkaisu FROM a_kommentti WHERE julkaisu = ".$i.";");
				$popularity->execute();
				$popularity_data = $popularity->fetch();
					if ($popularity_data['COUNT(julkaisu)'] > $mostPopular){
						$mostPopular=$popularity_data['COUNT(julkaisu)'];
						$popularId=$popularity_data['julkaisu'];
						
						//echo($popularity_data." ! ");
			...
			echo('Winner '.$popularTitle_data['title'].' with '. $mostPopular.' comments');
